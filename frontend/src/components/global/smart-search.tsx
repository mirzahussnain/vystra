...
export const SmartSearch = ({
  ...
  const [triggerAiSearch, { data: aiResults, isFetching: loading }] =
  ...
  const keywordResults = useMemo(() => {
    ...
    return searchAbleSegments
      .filter((segment) =>
        segment.searchBlob.includes(deferredSearch.toLowerCase()),
      )
      .slice(0, 10);
  ...
  useEffect(() => {
    if (isPro && aiResults && aiResults.results.length > 0) {
      toast.success(`AI Search Results for query ${aiResults.query} Loaded`, {
        description: `credits remaining: ${aiResults.remaining_credits} `,
      });
      filterTranscript(aiResults.results);
    }
